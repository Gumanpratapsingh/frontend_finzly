...
export class InvoiceGenerateComponent {
  ...
  fetchConnectionIds() {
    if (this.phoneForm.valid) {
      this.http.get<string[]>(`https://frontendfinzly-production.up.railway.app/api/customers/connection-ids/${this.phoneForm.value.phoneNumber}`)
        .subscribe(
          (ids) => {
  ...
  generateInvoice() {
    if (this.dateForm.valid && this.connectionForm.valid) {
      ...
      const url = `https://frontendfinzly-production.up.railway.app/api/invoices/generate?phoneNumber=${phoneNumber}&connectionId=${connectionId}&startDate=${startDate}&endDate=${endDate}`;
